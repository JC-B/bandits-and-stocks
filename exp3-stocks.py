...
payoffStats = lambda data, gamma: stats(exp3Stocks(data, gamma)[0] for _ in range(5000))


def runExperiment(table, gamma):
   print("(Expected payoff, variance) over 5000 trials is %r" % (prettyList(payoffStats(table, gamma)),))
...
if __name__ == "__main__":
   ...
   initgamma = .1
   #bestgamma = bestGamma(table)
   print("----F500----")
   #print("Initial Gamma : %.2f, best gamma is %.2f" % (initgamma, bestgamma))
   for gamma in [initgamma]: # [initgamma, bestgamma]:
       runExperiment(table, gamma)
       payoffGraph(table, list(sorted(table.keys())), cumulative=True, imgfile='images/exp3-f500-rewards-gamma-'+str(gamma)+'.png')
       #print(weightsStats(table, gamma))
    
   print()

   table2 = readInStockTable('stocks/random-stocks.csv')   
   initgamma = .1
   #bestgamma = bestGamma(table2)
   print("----Random Stocks----")
   #print("Initial Gamma : %.2f, best gamma is %.2f" % (initgamma, bestgamma))
   for gamma in [initgamma]: #[initgamma, bestgamma]:
       print("gamma is %.2f" % (gamma))
       runExperiment(table2, gamma)
       payoffGraph(table2, list(sorted(table2.keys())), cumulative=True, imgfile='images/exp3-random-stocks-rewards-gamma-'+str(gamma)+'.png')
       #print(weightsStats(table2, gamma))
